...
def save_model(model):
    ...
    result = mlflow.sklearn.log_model(
        sk_model=model, artifact_path="models", registered_model_name=MLFLOW_MODEL_NAME
    )
...
def register_model(client, version, alias: str):
    ...
    if alias not in ["challenger", "champion"]:
        raise ValueError(
            f"alias must be 'challenger' or 'champion', got {alias} instead"
        )

    client.set_registered_model_alias(
        name=MLFLOW_MODEL_NAME, alias=alias, version=version
    )
    client.set_model_version_tag(
        name=MLFLOW_MODEL_NAME, version=version, key="alias", value=alias
    )
...
def promote_challenger():
    ...
    try:
        ...
        version_champ = client.get_model_version_by_alias(
        ...
        client.delete_model_version_tag(
            name=MLFLOW_MODEL_NAME, version=version_champ, key="alias"
        )
    ...
    version_chall = client.get_model_version_by_alias(
        name=MLFLOW_MODEL_NAME, alias="challenger"
    ).version
    # set its alias as champion.
    client.set_registered_model_alias(
        name=MLFLOW_MODEL_NAME, alias="champion", version=version_chall
    )
    ...
    client.set_model_version_tag(
        name=MLFLOW_MODEL_NAME, version=version_chall, key="alias", value="champion"
    )
