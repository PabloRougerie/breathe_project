...
class PreprocessConfig:
    ...
    horizon: int = HORIZON
    features: list = field(
        default_factory=lambda: SELECTED_FEATURES
    )  # mutable default: field() required to avoid shared state between instances
...
def preprocessing_pipeline(
    ...
    if mode == "train":
        airqual_sensor_filtered = filter_sensors(
            df=airqual_no_neg,
    ...
    airqual_col_selected = filter_columns(
        df=airqual_sensor_filtered,
    ...
    print(
        f"✅ Preprocessing done — {dataset_metadata['n_rows']} rows, {dataset_metadata['n_features']} features | {dataset_metadata['date_start']} → {dataset_metadata['date_end']}"
    )
