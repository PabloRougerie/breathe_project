...
def average_feature_generation(df, shifts=[12, 13, 14]):
    ...
    df["lag_avg_14"] = pd.concat(
        [df.groupby("city")["pm25_avg"].shift(shift) for shift in shifts], axis=1
...
def std_feature_generation(df, time_window=7):
    ...
    df["week_std"] = df.groupby("city")["pm25_avg"].transform(
        lambda x: x.rolling(time_window).std()
    )
...
def feature_engineering(df, approach="custom"):
    ...
    if approach == "custom":
        df = average_feature_generation(
            df
        )  # average pm25 value in the past, on lag 13-15 by default
        df = std_feature_generation(
            df
        )  # add pm25 variability over most recent week by default
        ...
        raise ValueError(
            f"approach parameter must be either 'custom', 'all_lags_14' or 'all_lags_21', got {approach} instead"
        )
